...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 w-full h-[58px] z-[500] bg-[#12111a]/95 backdrop-blur-xl border-b border-white/10 flex items-center shadow-lg">
      <div className="flex justify-between items-center w-full max-w-[1560px] mx-auto px-4 md:px-8 h-full">
        {/* Logo */}
        <Link to="/" className="flex items-center space-x-2 shrink-0">
          <img src="/tatva_bg.png" alt="TATVA Logo" className="h-[30px] w-auto max-h-[30px] object-contain hover:opacity-90 transition-opacity" />
        </Link>

        {/* Desktop Dock Navigation (Home, Composer, My Circuits, Team) */}
          ...
          {isLoggedIn ? (
            <div className="relative">
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-primary-container text-white flex items-center justify-center font-label-sm text-label-sm font-bold hover:shadow-[0_0_15px_rgba(79,70,229,0.5)] transition-all"
                >
                  {user?.name?.charAt(0)?.toUpperCase() || 'U'}
                </button>
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-surface-container-high border border-outline-variant/30 rounded-xl shadow-xl overflow-hidden z-50">
            ...
            <>
              <button
                ...
                className="px-5 py-1.5 rounded-lg font-body-md text-on-surface text-sm border border-outline-variant hover:border-primary-container hover:bg-primary-container/10 transition-all duration-300"
              >
                Login
              </button>
              <button
                onClick={() => navigate('/auth?tab=register')}
                className="px-5 py-1.5 rounded-lg font-body-md text-sm text-white bg-primary-container hover:shadow-[0_0_15px_rgba(79,70,229,0.5)] hover:-translate-y-0.5 transition-all duration-300 border-t border-white/20"
              >
                Sign Up
              </button>
            </>
      ...
      {mobileOpen && (
        <div className="md:hidden absolute top-[58px] right-2 sm:right-4 w-[230px] bg-[#12111a]/98 border border-white/15 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] rounded-2xl z-[600] p-3.5 flex flex-col space-y-2 animate-fadeIn">
          <Link
            to="/"
